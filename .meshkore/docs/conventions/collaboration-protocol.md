---
title: "MeshKore collaboration protocol"
...
Canonical cluster config: **`.meshkore/public/cluster.yaml`**. Credentials: **`.meshkore.local`** and **`.meshkore/credentials/`** (never commit).
...
- **Read the inbox before writing code** when the user prompt references the cluster, MeshKore, or `.meshkore/docs/conventions/`.
...
- **`channel_id`:** take from `.meshkore/public/cluster.yaml` → `legacy_hub.channel_id`.
...
Align with Messaging: `plan-task-review@v1` (MeshKore hub channel).
...
- This folder (**`.meshkore/docs/conventions/`**) defines **how mesh traffic maps to Hyverk code**; numeric performance truth stays in inference module notes under `.meshkore/modules/inference/`.
